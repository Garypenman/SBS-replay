...
void TOFcal(const char *inputfilename="testconfig", const char *outputfilename="TOFcal_temp.root"){
  ...
  TH2D *htmean_hodoID_old = new TH2D("htmean_hodoID_old","OLD ;bar ID; bar t_{mean} [ns]", 90,-0.5,89.5, 150,-30,30);
  TH2D *htmean_hodoID_new = new TH2D("htmean_hodoID_new","NEW ;bar ID; bar t_{mean} [ns]", 90,-0.5,89.5, 150,-30,30);
  ...
  TH2D *htmean_trigRF_old = new TH2D("htmean_trigRF_old", " OLD; bar ID; bar t_{mean} + bb_{trig} - fmod{rf,2.004) ", 90,-0.5,89.5,150,-30,30);
  TH2D *htmean_trigRF_new = new TH2D("htmean_trigRF_new", " OLD; bar ID; bar t_{mean} + bb_{trig} - fmod{rf,2.004) ", 90,-0.5,89.5,150,-30,30);
  
  TH2D *htmean_hcalID_old = new TH2D("htmean_hcalID_old","OLD ;hcal ID; clus t_{mean} [ns]", 288,-0.5,287.5,150,-30,30);
  TH2D *htmean_hcalID_new = new TH2D("htmean_hcalID_new","OLD ;hcal ID; clus t_{mean} [ns]", 288,-0.5,287.5,150,-30,30);
  
  TH2D *htmean_hcale_old = new TH2D("htmean_hcale_old","OLD ;hcal edep [GeV]; clus t_{mean} [ns]", 150,0,0.5,150,-30,30);
  TH2D *htmean_hcale_new = new TH2D("htmean_hcale_new","OLD ;hcal edep [GeV]; clus t_{mean} [ns]", 150,0,0.5,150,-30,30);
  ...
  for( int iter=0; iter<2; iter++){
    if(iter==0) cout << "First run to get parameters" << endl;
    else cout << "Now second run to test our calibration" << endl;
    for( long ev=0; ev<nev; ev++ ){
    
      T->GetEntry(ev);
      if( ev % 1000 == 0 ){
	cout << "nevent = " << ev << ", run number = " << T->g_runnum << endl;
      }
    
      treenum = C->GetTreeNumber();
      if( ev == 0 || treenum != oldtreenum ){
	oldtreenum = treenum;
	GlobalCut->UpdateFormulaLeaves();
      ...
      //bbcal trigger time
      double trigtime = T->bb_gem_trigtime; 
    
      //rf time
      double rftime=-999.;
      for(int i=0; i<T->Ndata_bb_tdctrig_tdcelemID; i++){
	if(T->bb_tdctrig_tdcelemID[i]==4) rftime = T->bb_tdctrig_tdc[i];
      }
	...
	htmean_trigRF_old->Fill( ID, T->bb_hodotdc_clus_tmean[0] + trigtime - fmod(rftime,2.004) );
	htmean_trigRF_new->Fill( ID, 0.5*(tleft_CORR+tright_CORR) + trigtime - fmod(rftime,2.004) );
